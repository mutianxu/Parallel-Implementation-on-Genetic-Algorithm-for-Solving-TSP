...
class ComputePath:
    def __init__(self, distance, ID2Index, lives, num, i_d):
    ...
    def evaluation(self,lis,lc):
        ...
        for life in self.lives:
            ...
            sortlive.append(life)
            sortscore.append(score)
            if score > best_score:
                self.best = life
                best_score = score
        s_score=sortscore
        s_score=sorted(s_score)
        choose = s_score[25]

        for i in range(0,len(sortlive)):
            if len(miti)>=25:
                break
            if sortscore[i] < choose or sortscore[i] == choose:
                miti.append(sortlive[i])
        count=len(miti)
        lc.acquire()
        if len(lis)!=4*count:
            lis+=miti
            ...
            if self.id ==1:
                ...
                lis[count:2*count]=miti
            elif self.id==3:
                lis[2*count:3*count]=miti
            elif self.id==4:
                lis[3*count:]=miti
        self.miti_list=lis
        lc.release()
    ...
    def next_generation(self,lis,lc):
        self.evaluation(lis,lc)
        newLives = []
        newLives.append(self.best)
        # kai=time.time()
        inter=list(self.miti_list[:100])
    ...
    def GA(self,order_list,q,lis,lc):
        dis=0
        n=2000
        self.order = ['000'] + order_list + ['-1']
        self.geneLength=len(self.order)
        while n > 0:
            self.next_generation(lis,lc)
...
def parallel(order_list,distance_test_1,ID2Index):
    ...
    bry=mp.Manager()
    b_L=bry.list()
    st1 = mp.Process(target=initialpopulation, args=(num, length, a_L,))
    st2 = mp.Process(target=greedy, args=(order_list,ID2Index,distance_test_1,b_L,))
    st1.start()
    st2.start()
    st1.join()
    st2.join()
    ...
    array=ary.list()
    lock=mp.RLock()
    com1 = ComputePath(distance_test_1, ID2Index, l1, num / 4,1)
    com2 = ComputePath(distance_test_1, ID2Index, l2, num / 4,2)
    com3 = ComputePath(distance_test_1, ID2Index, l3, num / 4,3)
    com4 = ComputePath(distance_test_1, ID2Index, l4, num / 4,4)
    q1 = mp.Queue()
    q2 = mp.Queue()
    q3 = mp.Queue()
    q4 = mp.Queue()
    p1 = mp.Process(target=com1.GA, args=(order_list, q1,array,lock,))
    p2 = mp.Process(target=com2.GA, args=(order_list, q2,array,lock,))
    p3 = mp.Process(target=com3.GA, args=(order_list, q3,array,lock,))
    p4 = mp.Process(target=com4.GA, args=(order_list, q4,array,lock,))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
